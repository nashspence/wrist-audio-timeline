...
- `Timeline projection` is the integration step that converts upstream outputs into canonical `observation` and `evidence` objects.
- `Baseline context` and `Quality` create canonical `derived` objects.
- `Fusion` creates canonical `fused_interval` objects.
- `Narrative` enriches `fused_interval.narrative` and may also emit an optional narrative artifact.
...
|---|---|---|---|---|---|---|
| 1 | Register input | Ingest | CPU | none | creates `capture_session`, `sensor_stream`, primary-media `artifact` | all downstream stages |
| 2 | Extract signal features | DSP | CPU | Ingest | none directly; later projects to `audio_dsp_bin_observation` on `audio_base_500ms` | Timeline projection, Baseline context, Quality, Fusion |
| 3 | Transcribe speech | ASR | GPU | Ingest | none directly; later projects to `audio_asr_word_evidence` and `audio_asr_segment_evidence` | ASR correction, Timeline projection, Quality, Fusion |
| 4 | Segment speakers | Diarization | GPU | Ingest | none directly; later projects to `audio_diarization_segment_evidence` | Emotion, Speaker ID, Timeline projection, Quality, Fusion |
| 5 | Detect environment events | SED | GPU | Ingest | none directly; later projects to `audio_sound_event_segment_evidence` | Timeline projection, Baseline context, Quality, Fusion |
| 6 | Build coarse context windows | Context windows | GPU | Ingest | none directly; later projects to `audio_context_window_evidence` on `audio_context_30s_15s` | Timeline projection, Baseline context, Fusion |
| 7 | Detect speech affect | Emotion | GPU | Diarization | none directly; later projects to `audio_emotion_window_evidence` and optionally `audio_emotion_segment_evidence` | Timeline projection, Quality, Fusion |
| 8 | Resolve speakers | Speaker ID | GPU | Diarization | none directly; later projects to `audio_speaker_identification_evidence` | Timeline projection, Fusion |
| 9 | Correct transcript | ASR correction | GPU | ASR | none directly; later projects to `audio_asr_correction_evidence` | Timeline projection, Fusion |
| 10 | Project to canonical timeline objects | Timeline projection | CPU | DSP, ASR, Diarization, SED, Context windows, Emotion, optional Speaker ID, optional ASR correction | creates canonical `observation` and `evidence` objects plus typed refs | Baseline context, Quality, Fusion |
| 11 | Merge broad context into durable segments | Baseline context | CPU | Timeline projection | creates `context_segment` and `context_change_marker` | Quality, Fusion |
| 12 | Score reliability and usability | Quality | CPU | Timeline projection, Baseline context | creates `quality_bin`; may update confidence bundles on existing canonical objects | Fusion |
| 13 | Build durable downstream intervals | Fusion | CPU | Timeline projection, Baseline context, Quality | creates `fused_interval` | Narrative, final output |
| 14 | Write narrative summaries | Narrative | CPU initially | Fusion | updates `fused_interval.narrative`; may emit optional narrative artifact | final output |
...
- Emotion runs after Diarization because diarized spans improve speech affect interpretation.
- Speaker ID is optional and depends on Diarization plus enrollment.
- ASR correction is optional and depends on ASR. It runs before Timeline projection so corrected transcript evidence is available to fusion.
...
| Diarization | `BUT-FIT/diarizen-wavlm-large-s80-md-v2` | Speaker-homogeneous spans | primary-media artifact | diarization native outputs | `audio_diarization_segment_evidence` | GPU | basis for speaker-aware emotion and speaker ID |
| SED | `atst_as2M.ckpt` + `Stage2_wo_ext.ckpt` | Non-speech and environment events | primary-media artifact | sound-event native outputs, optional `native_output` artifact | `audio_sound_event_segment_evidence` | GPU | context layer only |
| Context windows | `Qwen/Qwen2.5-Omni-7B` | Coarse semantic scene windows | primary-media artifact, fixed 30 s / 15 s windows | context-window native outputs, optional raw-output artifact | `audio_context_window_evidence` | GPU | broad prior, not final segmentation; uses canonical grid `audio_context_30s_15s` |
| Emotion | `3loi/SER-Odyssey-Baseline-WavLM-Categorical` + `3loi/SER-Odyssey-Baseline-WavLM-Multi-Attributes` | Emotional evidence for speech | primary-media artifact, preferably diarized spans | emotion native outputs, optional decoded emotion artifact | `audio_emotion_window_evidence`, optionally `audio_emotion_segment_evidence` | GPU | best after Diarization |
| Speaker ID | `nvidia/speakerverification_en_titanet_large` | Map speakers to enrolled identities | diarization outputs, primary-media artifact, enrolled speakers | speaker-identification native outputs | `audio_speaker_identification_evidence` | GPU | optional if no enrollment |
...
| Timeline projection | custom projection service | Convert upstream outputs into canonical timeline objects and typed refs | DSP, ASR, Diarization, SED, Context windows, Emotion, optional Speaker ID, optional ASR correction outputs | canonical bundle updates | `audio_dsp_bin_observation`, `audio_context_window_evidence`, `audio_asr_word_evidence`, `audio_asr_segment_evidence`, `audio_asr_correction_evidence`, `audio_diarization_segment_evidence`, `audio_speaker_identification_evidence`, `audio_emotion_window_evidence`, `audio_emotion_segment_evidence`, `audio_sound_event_segment_evidence` | CPU | integration layer; preserves typed refs, lineage, timebase, and grid semantics |
...
| Quality | custom scoring service | Score suitability and reliability by timeline bin | projected observations/evidence, context segments | canonical bundle updates | `quality_bin` and optional confidence updates on existing objects | CPU | uses reserved quality metric and usability vocabularies |
| Fusion | custom fusion service | Build durable downstream interpretation intervals | projected observations/evidence, context segments, quality bins | canonical bundle updates | `fused_interval` | CPU | deterministic first |
...
| Speaker ID | diarization evidence | anonymous speaker labels |
...
Use the same order as the execution order above:
